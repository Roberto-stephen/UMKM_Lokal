...
if ($stmt->rowCount() == 0) { echo '<div class="container"><div class="alert alert-danger" style="margin-top:30px;">Produk tidak ditemukan.</div></div>'; include $tpl.'footer.php'; exit(); }
$item = $stmt->fetch();

$errors = []; $success = '';
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    $name=$trim($_POST['name'] ?? ''); $name=trim($_POST['name']);
    $desc=trim($_POST['description']); $price=intval($_POST['price']);
    $contact=trim($_POST['contact']); $cat_id=intval($_POST['cat_id']);
    $cbf_kat=trim($_POST['cbf_kategori']); $cbf_rasa=trim($_POST['cbf_rasa']);
    $cbf_bahan=trim($_POST['cbf_bahan']); $cbf_pedas=trim($_POST['cbf_kepedasan']);
    ...
    if (!empty($_FILES['picture']['name'])) {
        ...
        if (!in_array($ext,['jpg','jpeg','png','gif','webp'])) { $errors[]='Format gambar tidak didukung.'; }
        elseif ($_FILES['picture']['size']>3000000) { $errors[]='Ukuran gambar maksimal 3MB.'; }
        else {
    ...
    if (empty($errors)) {
        $upd=$con->prepare("UPDATE items SET Name=?,Description=?,Price=?,Cat_ID=?,contact=?,picture=?,cbf_kategori=?,cbf_rasa=?,cbf_bahan=?,cbf_kepedasan=?,Approve=0 WHERE Item_ID=? AND Member_ID=?");
        $upd->execute([htmlspecialchars($name),htmlspecialchars($desc),$price,$cat_id,htmlspecialchars($contact),$picture,$cbf_kat,$cbf_rasa,$cbf_bahan,$cbf_pedas,$id,$_SESSION['uid']]);
  ...
  <div class="col-md-6">
    <div class="form-group">
      ...
      <select class="form-control" name="cat_id" required>
        ...
        <option value="<?php echo $cat['ID'] ?>" <?php echo $cat['ID']==$item['Cat_ID']?'selected':'' ?>><?php echo htmlspecialchars($cat['Name']) ?></option>
        <?php endforeach; ?>
      </select>
    </div>
  </div>
...
<div class="form-group">
  <label style="font-size:12px;font-weight:600;color:#4A4A6A;display:block;margin-bottom:5px;">FOTO PRODUK (kosongkan jika tidak diganti)</label>
  <?php if ($item['picture'] && $item['picture']!='default.png'): ?>
  <img src="admin/uploads/items/<?php echo htmlspecialchars($item['picture']) ?>" style="width:80px;height:80px;object-fit:cover;border-radius:8px;margin-bottom:8px;display:block;">
  <?php endif; ?>
...
<div class="form-group">
  ...
  <input class="form-control" type="text" name="cbf_rasa" required value="<?php echo htmlspecialchars($item['cbf_rasa']) ?>">
</div>
<div class="form-group">
  <label style="font-size:12px;font-weight:600;color:#4A4A6A;display:block;margin-bottom:5px;">BAHAN UTAMA *</label>
  <input class="form-control" type="text" name="cbf_bahan" required value="<?php echo htmlspecialchars($item['cbf_bahan']) ?>">
</div>
